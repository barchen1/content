    ...
    try:
        ...
        if is_ami and mockable_tests:
            ...
            while not mockable_tests_queue.empty():
                ...
                test_playbook_id = t.get('playbookID')
                # If playbookID is already in executed_in_current_round- it means all tests the could have been locked
                # have been locked and executed, and that this is a new round of tests that were locked in the previous
                # round.
                # In that case we want to sleep for a minute and clear the current round tests set for the new round
                if test_playbook_id in executed_in_current_round:
                    executed_in_current_round = set()
                    time.sleep(60)
                executed_in_current_round.add(test_playbook_id)
...
def initialize_queue_and_executed_tests_set(unmockable_tests):
    mockable_tests_queue = SimpleQueue()
    already_executed_test_playbooks = set()
    for t in unmockable_tests:
        mockable_tests_queue.put(t)
    return already_executed_test_playbooks, mockable_tests_queue
...
def safe_lock_integrations(test_timeout: int,
    ...
    if should_lock_integrations:
        integration_names = get_integrations_list(integrations_details)
        prints_manager.add_print_job(
